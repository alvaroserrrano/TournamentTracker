    ...
    public static class TournamentLogic
    {
        ...
        public static void UpdateTournamentResults(TournamentModel model)
        {
            ...
            foreach (List<MatchupModel> round in model.Rounds)
            {
                ...
                {
                    if (rm.Winner == null && (rm.Entries.Any(x => x.Score != 0) || rm.Entrys.Count == 1))
                    {
                        toScore.Add(rm);
                    }
                }
        ...
        private static void AlertPersonToNewRound(PersonModel person, string teamName, MatchupEntry competitor)
        {
        ...
        private static void CompleteTournament(TournamentModel model)
        {
            ...
            if (model.Prizes.Count > 0)
            {
                ...
                PrizeModel secoundPlacePrize = model.Prizes.Where(x => x.PlaceNumber == 2).FirstOrDefault();
                if (firstPlacePrize != null)
                {
        ...
        private static void AdvanceWinners(List<MatchupModel> models, Tournament tournament)
        {
            foreach (MatchupModel m in models)
            {
